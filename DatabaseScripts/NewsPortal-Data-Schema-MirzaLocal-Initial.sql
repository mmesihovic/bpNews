...
create or replace trigger random_trigger
...
create index random_index on Roles (Name);

create or replace procedure random_procedura (role_id IN NUMBER) AS
  BEGIN
    DELETE FROM Roles WHERE Roles.ID = random_procedura.role_id;
  end;
